...
Global $LC_Retour, $LO_XML;
	...
	if ($LN_OK == 5){
		$LC_Retour = "Vos données sont valides.<br> Vous pouvez demander vos résultats en sélectionnant un document à gauche.";
		return "OK" . $LC_Retour;
	}
	else
	{
...
Global $LC_Retour, $LO_XML;
	...
	$LB_Result= true;
		...
		if (TOOL_DecodeNum($LO_XML->simulation->duree) < TOOL_DecodeNum($LO_XML->simulation->credit->duree)/12) 
	{
		$LC_Texte = 	//"~NomPrenom~, \n" .
					"La durée de la simulation doit être valide (supérieure ou égale à la durée du crédit)";
		$LC_Texte = VERIF_Texte($LC_Texte);
		$LC_Retour= $LC_Retour . $LC_Texte . "<br/>";	
		$LB_Result= false;
	}	
	
	if (TOOL_DecodeNum($LO_XML->simulation->duree) > 50) 
	{
		$LC_Texte = 	//"~NomPrenom~, \n" .
					"La durée de la simulation doit être valide (inférieure à 50 ans)";
		$LC_Texte = VERIF_Texte($LC_Texte);
		$LC_Retour= $LC_Retour . $LC_Texte . "<br/>";	
		$LB_Result= false;
	}
